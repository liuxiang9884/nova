...
- **作者节点池在构造时预分配，其分配和初始化被排除；Abseil/STL 插入过程中的实际分配仍计入。Radix 的常驻目录构造被排除，但插入时的页/叶分配、初始化和移动，以及删除时的叶移动、回收和清空池都计入。** 这是保留原实现分配策略的操作成本对比，不是公平化 allocator 后的算法成本，也不是端到端建表成本。
...
`radix_bitmap_set.hpp` 是本实验自主实现。作者未提供可核对的 radix 源码；这里的布局和优化不代表复现作者算法。前六轮见 [OPTIMIZATION.md](OPTIMIZATION.md)，本次布局/页池实验见 [LAYOUT_OPTIMIZATION.md](LAYOUT_OPTIMIZATION.md)。
...
- 稀疏页内联 16 个叶字，超过后容量按倍数增长至 64。新增第 65 个非空叶字时，分配 1,024 个直接索引叶字的未初始化存储，只复制原有有效叶字，然后释放紧凑数组的溢出存储。扩容、转换和有效叶初始化均在插入计时区间内，未借助构造参数预分配。
- 稀疏模式新增或移除叶字时更新后续前缀。编译开启 AVX2 时用一次 256 位向量操作更新 16 个计数；其余平台使用标量循环，功能与数据布局一致。稠密模式直接索引叶字，不移动数组、不维护或读取已失效的前缀计数。
- 稠密数组的有效性由第三层占用位图控制，所有读取先确认占用；往空叶插入时先完整写入叶字，再设置占用位。无需清零未使用槽，删除后重插也不会暴露旧位。
- 页对象通过延迟创建的 256 页 slab 分配和复用。首次插入才分配 slab，实际使用的页才初始化；没有构造期预分配或按数据集规模预留。
...
slab 和扩容数组由 RAII 管理；页使用 placement construction 与显式销毁，分配成功后才发布摘要和计数。空页立即销毁并释放其叶数组，页槽回到池内复用；slab 保留至整个集合为空或被销毁。单个页为空不等于向 allocator 归还 slab。数组在页非空时保留历史容量，稠密页也不降级，避免反复收缩或转换。极密集数据仍可占用数百 MiB；转换阈值影响吞吐与空间，因此不能假定所有数据规模均受益。

`StorageBytes()` 和查询 JSON 中的 `storage_bytes` 包含对象、slab 指针数组容量、所有已分配页槽（包括空闲槽）及活页的叶数组容量，不含 allocator 自身元数据/保留内存、数据集和其他容器，不是进程 RSS。内联空间计入页大小，溢出数组另计，避免重复统计。存储统计发生在查询计时之前，不计入查找成本。

## 结构诊断

`nova_radix_diagnostics` 是单独启用计数器的构建，统计实际页/叶分配、slab 分配、字节搬移、前缀更新和稠密转换；不输出性能时间，也不将计数器编译进正式 benchmark：

```bash
cmake --build build/set-release --target nova_radix_diagnostics
build/set-release/benchmark/btree_set/nova_radix_diagnostics 1000000
```

`page_allocations/page_bytes` 是逻辑页构造的数量/字节，`pool_allocations/pool_bytes` 是其底层 slab 分配量，两者不要相加；`leaf_bytes` 是累计分配流量，`storage_bytes` 是当前容量。它们不是 CPU 周期占比。
